<?php
$oldBlade = file_get_contents('resources/views/program-strategis.blade.php');
...
<main class="flex-1 overflow-y-auto p-8 relative bg-[#F8F9FA]">
                ...
                <div class="relative inline-block text-left">
                    ...
                    <div id="dropdownOpsiSuper" class="hidden origin-top-right absolute right-0 mt-2 w-56 rounded-xl shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                        <div class="py-1" role="menu">
                            <button type="button" onclick="openModalTambahMaster()" class="w-full text-left px-4 py-2.5 text-xs text-gray-700 hover:bg-orange-50 hover:text-orange-700 flex items-center gap-2">
    ...
    <x-modal id="modalAturKolom" title="Pengaturan Kolom Tambahan" description="Tambah atau hapus kolom khusus pada tabel Program Strategis.">
        <form action="{{ route('dynamic-column.store') }}" method="POST" class="mb-6">
            @csrf
                ...
                <div class="grid grid-cols-2 gap-3">
                    @foreach(\$kolomDinamis as \$kol)
                        <div class="flex justify-between items-center bg-gray-50 px-3 py-2 rounded border border-gray-200">
                            ...
                            <button type="button" onclick="openDeleteModal('modalHapusKolom', '{{ route('dynamic-column.destroy', \$kol->id) }}')" class="text-red-500 hover:text-red-700 p-1">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                            </button>
                        </div>
                    @endforeach
...
file_put_contents('resources/views/program-strategis.blade.php', $newBlade);
echo "program-strategis.blade.php reconstructed.\n";
