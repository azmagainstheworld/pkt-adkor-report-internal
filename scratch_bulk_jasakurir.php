<?php
$content = file_get_contents('resources/views/jasakurir.blade.php');
...
EOD;
$content = str_replace('<x-button variant="primary" onclick="bukaModalTambahData()"', $bulkBtn . "\n                <x-button variant=\"primary\" onclick=\"bukaModalTambahData()\"", $content);

// 3. Form and Action Bar
$actionBar = <<<EOD
        <form id="bulkDeleteForm" action="{{ route('jasakurir.destroyBulk') }}" method="POST" onsubmit="return confirm('Hapus data terpilih?')">
            @csrf
            @method('DELETE')
...
$content = preg_replace('/<\/x-table>\s*<\/div>\s*<!-- PAGINATION/s', "</x-table>\n            </div>\n        </form>\n        <!-- PAGINATION", $content, 1);

// 4. Update Headers
$content = str_replace("\$headers = ['Tahun', 'Bulan'];", "\$headers = ['<input type=\"checkbox\" id=\"selectAllBulk\" onclick=\"toggleSelectAll()\">', 'Tahun', 'Bulan'];", $content);

// 5. Update row - composite key for checkbox
$content = preg_replace('/(<tr class="hover:bg-gray-50 transition-colors text-sm border-b border-gray-100 last:border-0">\s*<td class="px-6 py-4 text-gray-700 font-medium whitespace-nowrap">{{ \$row\[\'tahun\'\] }})/s', 
    "<tr class=\"hover:bg-gray-50 transition-colors text-sm border-b border-gray-100 last:border-0\">\n                        <td class=\"px-6 py-4 text-center align-middle\"><input type=\"checkbox\" name=\"ids[]\" class=\"cb-bulk\" value=\"{{ \$row['tahun'] }}|{{ \$row['bulan'] }}\" onclick=\"toggleCheckbox()\"></td>\n                        <td class=\"px-6 py-4 text-gray-700 font-medium whitespace-nowrap\">{{ \$row['tahun'] }}", $content);

// Update colspan empty state
$content = preg_replace('/(<tr><td colspan="{{ count\(\$headers\) )("\s*class="px-6 py-10 text-center text-gray-500">)/', '${1}${2}', $content);
...
file_put_contents('resources/views/jasakurir.blade.php', $content);
echo "Jasa Kurir view updated for bulk delete.\n";
