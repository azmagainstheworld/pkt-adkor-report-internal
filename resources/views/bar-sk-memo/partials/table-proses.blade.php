...
<x-card class="!rounded-xl overflow-visible !p-0 shadow-sm border border-gray-100 bg-white relative">
    <div class="p-5 border-b border-gray-100 bg-white flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-t-xl z-[30] relative">
        <h3 class="font-bold text-gray-900 text-base uppercase">TABEL BAR SK MEMO PROSES</h3>
        
        <div class="flex items-center gap-2 flex-wrap">
            <div class="relative inline-block text-left overflow-visible z-[50]">
                <button type="button" onclick="toggleDropdown('dropdownOpsiProses')" class="inline-flex justify-center items-center gap-1.5 rounded-xl border border-gray-300 shadow-sm px-3 py-1.5 bg-white text-xs font-medium text-gray-700 hover:bg-gray-50 focus:outline-none transition-colors">
                ...
                <div id="dropdownOpsiProses" class="hidden absolute right-0 z-[50] mt-2 w-56 origin-top-right rounded-xl bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none overflow-visible transition-all">
                    <div class="px-4 py-2 bg-gray-50 border-b border-gray-100">
                        <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Kelola Data Excel</p>
                    </div>
                    <div class="py-1">
                        <button type="button" onclick="openModal('modalImportProses'); toggleDropdown('dropdownOpsiProses')" class="w-full text-left text-gray-700 px-4 py-2 text-xs hover:bg-green-50 flex items-center gap-2 font-medium transition-colors">
                        ...
                        <a href="{{ route('bar-sk-memo.export.excel', ['tahun' => $filterTahun, 'bulan' => $filterBulan, 'tipe' => 'proses']) }}" class="text-gray-700 px-4 py-2 text-xs hover:bg-green-50 flex items-center gap-2 font-medium transition-colors border-t border-gray-50">
                    ...
                    <div class="py-1">
                        <a href="{{ route('bar-sk-memo.export.pdf', ['tahun' => $filterTahun, 'bulan' => $filterBulan, 'tipe' => 'proses']) }}" target="_blank" class="text-gray-700 px-4 py-2 text-xs hover:bg-red-50 flex items-center gap-2 font-medium transition-colors">
                    ...
                    <div class="py-1">
                        ...
                        <button type="button" onclick="openModal('modalAturKolomProses'); toggleDropdown('dropdownOpsiProses')" class="text-gray-700 w-full text-left px-4 py-2 text-xs hover:bg-gray-50 flex items-center gap-2 font-medium transition-colors">
                        ...
                        @else
                        <span class="text-gray-300 w-full px-4 py-2 text-xs flex items-center gap-2 cursor-not-allowed select-none">
            ...
            <button type="button" id="btnModeBulkProses" onclick="toggleBulkMode('proses')" class="inline-flex justify-center items-center gap-1.5 rounded-xl border border-red-300 shadow-sm px-3 py-1.5 bg-white text-xs font-medium text-red-600 hover:bg-red-50 transition-colors">
                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                Hapus Semua
            </button>

            <x-button variant="primary" onclick="openModalTambah('Proses')" class="!py-1.5 !px-3 text-xs flex items-center gap-1"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg> Tambah Data Proses</x-button>
        </div>
    </div>

    <form id="bulkDeleteProsesForm" action="{{ route('bar-sk-memo.destroyProses') }}" method="POST">
        @csrf @method('DELETE')
        
        <div id="btnGroupProses" class="hidden flex justify-between items-center px-4 py-2 bg-red-50 border-b border-red-100">
            <span class="text-xs text-red-600 font-semibold">Data terpilih untuk dihapus</span>
            <div class="flex gap-2">
                <button type="button" onclick="cancelAll('proses')" class="px-3 py-1 bg-gray-500 text-white text-xs font-medium rounded hover:bg-gray-600 transition-colors">Batal</button>
        ...
        <div id="tableContainerProses" class="hide-bulk-proses overflow-x-auto">
            @php
                $headProses = ["<input type=\"checkbox\" id=\"selectAllProses\" class=\"w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500\" onclick=\"toggleSelectAll('proses')\">", 'Tahun', 'Bulan', 'Proses SKD Kep. Bersama', 'Proses SKD Non Ratifikasi', 'Proses SKD Ratifikasi', 'Proses Memo Direksi', 'Proses BAR Monitoring', 'Proses BAR Manajemen'];
                if(isset($kolomDinamisProses)) { foreach($kolomDinamisProses as $k) { $headProses[] = $k->nama_kolom; } }
                $headProses[] = 'Aksi';
            @endphp

            <x-table :headers="$headProses">
                @forelse($dataProses as $row)
                    ...
                    <tr class="hover:bg-gray-50 transition-colors text-xs whitespace-nowrap">
                        <td class="px-4 py-3 text-center"><input type="checkbox" name="ids[]" class="cb-proses w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" value="{{ $row->id }}" onclick="toggleCheckbox('proses')"></td>
                ...
                @empty
                    <tr><td colspan="{{ count($headProses) + 1 }}" class="px-6 py-10 text-center text-gray-500 text-sm">Data kosong.</td></tr>
                @endforelse
                
                @if(count($dataProses) > 0)
