...
<main class="flex-1 overflow-y-auto p-8 relative bg-[#F8F9FA]">
    ...
    <x-modal id="modalTambah" title="Tambah Data Dokumen" description="Pilih jenis dokumen dan masukkan jumlahnya.">
        <form action="{{ route('pa-teknik.storeDokumen') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 novalidate-form" novalidate>
            @csrf
...
<script>
    ...
    function editBulan(kelompok, tahun, bulan, itemsJson) {
        ...
        masters.forEach(m => {
            ...
            // TRICK 0 DI SINI UNTUK INPUT DINAMIS
            container.innerHTML += `
